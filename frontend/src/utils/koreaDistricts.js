/**
 * 대한민국 전국 17개 광역시·도 및 시군구 표준 데이터
 * 로그인 전(회원가입/소셜가입 추가정보)에도 네트워크나 토큰 없이 100% 즉시 선택할 수 있도록 보장합니다.
 */
...
export const KOREA_DISTRICTS = {
  ...
  경기도: [
    ...
    '안성시', '안양시 동안구', '안양시 만안구', '양주시', '양평군', '여주시', '연천군',
    '오산시', '용인시 기흥구', '용인시 수지구', '용인시 처인구', '의왕시', '의정부시',
...
/**
 * 시·도에 해당하는 시군구 목록을 반환합니다.
 * @param {string} provinceName
 * @returns {Array<{regionId: string, regionName: string}>}
 */
export function getDistrictsByProvince(provinceName) {
  const names = KOREA_DISTRICTS[provinceName] || [];
  return names.map((name, idx) => ({
    regionId: `${provinceName}_${idx}`,
    regionName: name,
  }));
}
